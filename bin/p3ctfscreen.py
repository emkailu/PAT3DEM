...
def main():
	...
	Then use this script to check if the maxres is too low.
	"""
	
	args_def = {'cutoff':10}	
	parser = argparse.ArgumentParser()
	parser.add_argument("ctfout", nargs='*', help="specify ctf output.txt to be screened")
	parser.add_argument("-c", "--cutoff", type=float, help="specify cutoff of maxres, by default {}".format(args_def['cutoff']))
	...
	for ctfout in args.ctfout:
		with open(ctfout) as ctf_read:
			maxres = float(ctf_read.readlines()[3].replace(';', '').split()[9])
		if maxres > args.cutoff:
			print ctfout		
